...
def test_strict_mode_improper_order():
    errors = run_validator_for_test_file("strict_errored.py", strict_mode=True)
    assert len(errors) == 2


def test_strict_mode_no_errors():
    assert not run_validator_for_test_file("strict_ok.py", strict_mode=True)
...
def test_strict_mode_with_private_errored():
    errors = run_validator_for_test_file("private_strict_errored.py", strict_mode=True)
    assert len(errors) == 3


def test_strict_mode_with_private_no_errors():
    assert not run_validator_for_test_file("private_strict_ok.py", strict_mode=True)
...
def test_warning_if_both_strict_mode_and_configurable_order_defined():
    with warnings.catch_warnings(record=True) as w:
        run_validator_for_test_file("ok.py", strict_mode=True, attributes_order=["nested_class", "field", "method"])
        assert len(w) == 1
