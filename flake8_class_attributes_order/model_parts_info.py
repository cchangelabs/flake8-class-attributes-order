...
def get_funcdef_type_by_decorator_info(node, decorator_names_to_types_map: dict[str, str]) -> str | None:
    for decorator_info in node.decorator_list:
        if isinstance(decorator_info, ast.Name) and decorator_info.id in decorator_names_to_types_map:
            ...
            return funcdef
        elif isinstance(decorator_info, ast.Attribute) and decorator_info.attr in {"setter", "deleter"}:
            if node.name.startswith("__"):
